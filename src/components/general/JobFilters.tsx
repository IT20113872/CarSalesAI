...
export function JobFilters() {
  ...
  // Local state for salary inputs to avoid laggy typing
  const [minSalaryInput, setMinSalaryInput] = useState(currentMinSalary);
  const [maxSalaryInput, setMaxSalaryInput] = useState(currentMaxSalary);

  // Keep local salary inputs in sync when URL changes
  useEffect(() => {
    setMinSalaryInput(currentMinSalary);
  }, [currentMinSalary]);

  useEffect(() => {
    setMaxSalaryInput(currentMaxSalary);
  }, [currentMaxSalary]);
  ...
  // Apply salary filter button handler
  const applySalaryFilter = () => {
    const params = new URLSearchParams(searchParams.toString());

    if (minSalaryInput) {
      params.set("minSalary", minSalaryInput);
    } else {
      params.delete("minSalary");
    }

    if (maxSalaryInput) {
      params.set("maxSalary", maxSalaryInput);
    } else {
    ...
    <Card className="col-span-1 h-fit">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="mt-4">
            <Button onClick={applySalaryFilter}>Apply Salary Filter</Button>
          </div>
        </div>
      </CardContent>
    </Card>
